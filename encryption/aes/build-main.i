...
extern void *(*__morecore) (ptrdiff_t __size) __attribute__ ((__deprecated__));


extern void *__default_morecore (ptrdiff_t __size)
__attribute__ ((__nothrow__ , __leaf__)) __attribute__ ((__malloc__)) __attribute__ ((__deprecated__));
...
rcon(int,int);
unsigned char**
KeyExpansion(unsigned char**,size_t);
...
void
AddRoundKey(unsigned char***, unsigned char**,size_t);
...
# 18 "main.c"
 ...
 size_t ANZAHLDERBLOCKE = RIGHT_SIZE_TEXT/16;

 text = (unsigned char*)realloc(text, RIGHT_SIZE_TEXT*sizeof(unsigned char));
 for(i = SIZE_TEXT;i<RIGHT_SIZE_TEXT;i++) text[i] = 0;

 unsigned char* key = (unsigned char*)malloc(4*4*sizeof(unsigned char) + 1);
 strcpy(key, "clave de 128bits");

 unsigned char** key4x4;
 unsigned char** kn;

 key4x4 = (unsigned char**)malloc(4*sizeof(unsigned char*));
 kn = (unsigned char**)malloc(4*sizeof(unsigned char*));
 for(i = 0;i<4;i++){
  key4x4[i] = (unsigned char*)malloc(4*sizeof(unsigned char));
  kn[i] = (unsigned char*)malloc(4*sizeof(unsigned char));
  for(j = 0;j<4;j++){
   key4x4[i][j] = key[i+4*j];
   kn[i][j] = key[i+4*j];
  }
 }
 free(key);
 ...
 for(i = 0;i<ANZAHLDERBLOCKE;i++){
  p3[i] = (unsigned char**)malloc(4*sizeof(unsigned char*));
  for(j = 0;j<4;j++){
   p3[i][j] = (unsigned char*)malloc(4*sizeof(unsigned char));
   for(k = 0;k<4;k++){
    p3[i][j][k] = text[i*16 +j+4*k];
   }
  }
 }
 ...
 for(i = 1; i <= numberRounds; i++){
  key_rounds[i - 1] = (unsigned char**)malloc(4*sizeof(unsigned char*));
